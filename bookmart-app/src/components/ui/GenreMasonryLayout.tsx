...
const GenreMasonryLayout = memo(({ genreTitle }: GenreMasonryLayoutProps) => {
  ...
  const renderBookCard = ({ item }: { item: any }) => (
    <MasonryBookCard book={item} navigation={navigation} />
  );

  const processedData = useMemo(() => {
    const listItems = [
      { type: "header" as const },
      ...genreBooks.map((book: any) => ({ type: "book" as const, book })),
    ];
    return listItems;
  }, [genreBooks]);
  ...
  const renderItem = ({ item }: { item: any }) => {
    ...
    switch (item.type) {
      case "header":
        return null;
      case "book":
        return <View style={wrapperStyle}>{renderBookCard({ item: item.book })}</View>;
      default:
        return null;
    }
  };
